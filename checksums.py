...
def checksum_file_hexdigest(fn):
    ...
    with open(fn,'rb') as f:
        f.read(chunksize)
        sha.update(chunksize)
    return sha.hexdigest()
    

def gen_folder_checksum(startpath,checksumoutfile):
    file_list,dir_list = get_files_directory(startpath)
    full_header = str(file_list)+'|'+str(dir_list)
    header_length = len(full_header)
    sha = hashlib.sha256()
    ...
    with open(checksumoutfile,'wb') as storage_file:
        #check for length mismatch
        checksumoutfile.write(full_header + str('\n'))
        sha.update(full_header)
        
        checksumoutfile.write('folder_structure\t' + str(sha.hexdigest()) + str('\n'))

        for current_file_header in file_list:
            checksumoutfile.write(fn + '\t' + checksum_file_hexdigest(fn) + '\n')
